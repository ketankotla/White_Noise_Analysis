...
import sys
sys.path.append("..")
from windowing import simple_rolling_window
...
def softplus(x,a,b,c,d,k,**kwargs):
...
def preprocess_traces(
    ...
    """
    preprocess traces for temporal filter extraction
    ...
    cut_traces
    cut_stimulus
    vals: dict of rate, cut trace mean and cut stimulus mean
    """
    ...
    print('returns cut_vals dict with keys "rate","cut_trace_mean" and "cut_stimulus_mean"')
...
class TemporalFilterExtraction(BaseEstimator, RegressorMixin):
    ...
    #
    #     """wrapper used for get_nonlinearity"""
    #
    #     sig = sigmoid(x,k,L,x0,b)
    #
    #     return sig
    #
    ...
    def fit(self, X, y, fit_nonlinearity=False):
        ...
        # Note that this does NOT return the temporal vector at the max amplitude
        # this means that the outer product of the temporal filter amplitude *
        # the spatial filter is not at the "correct" scale
    ...
    def get_nonlinearity(self, y, y_pred):
        ...
        if self.nonlin_method is None or self.nonlin_method == 'interp':
            ...
            popt, pcov = curve_fit(sigmoid, xdata, ydata, p0, sigma = sigma,maxfev=100000)
            #self.popt = popt
            ...
            nonlinearity = lambda x: sigmoid(x,**sigma_params)
            ...
            # was self._softplus
            popt, pcov = curve_fit(softplus, xdata, ydata, p0,sigma=sigma,bounds=bounds,maxfev=100000)
            #self.popt = popt
            print('p0: ',p0)
            print('popt: ',popt)
            soft_params={}
            ...
            # return a function, like interp1d.
            # the lambda function allows "currying"
            nonlinearity = lambda x: softplus(x,**soft_params)
